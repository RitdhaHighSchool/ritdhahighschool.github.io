	...
	$.fn.countdown = function (options, callback) {
		...
		function countdown() {
			...
			var text_days = (days === 1) ? settings.day : settings.days,
				...
				text_milliseconds = (milliseconds === 1) ? settings.millisecond : settings.milliseconds;
			...
			if (days >= 1) {
				...
				container.find('#milliseconds').css( "display", "inline" );				
			}
			if (hours >= 1 || days >= 1) {
				container.find('.hours').text(hours);
				container.find('.hours_text').text(text_hours);
			} else {
				container.find('#hours').css( "display", "none" );
			}
			if (minutes >= 1) {
				container.find('.minutes').text(minutes);
				container.find('.minutes_text').text(text_minutes);
			} else {
				container.find('#minutes').css( "display", "none" );
			}
		...
		// start
		var interval = setInterval(countdown, 66);
	};
